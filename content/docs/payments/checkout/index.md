...
Шаблон инвойса позволяет совершить более одной оплаты по одному и тому же [счёту](https://developer.rbk.money/docs/payments/overview/#invoice) в рамках заданного срока действия.
...
При оплате шаблона инвойса покупатель может самостоятельно указать [сумму](#htmlJsTemplate). Сумму допускается предзаполнить, при этом данное поле на форме останется доступным для редактирования.
...
Для того чтобы позволить покупателю самостоятельно ввести сумму оплаты, необходимо передать нижеописанный параметр.
